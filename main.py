...
for headline, is_sarcastic in data:
    ...
    # append The label (is_sarcastic) to the y list
...
# Train an SVM classifier using the proposed features Support Vector Machine (SVM)
...
X_ngram_reduced_train, X_ngram_reduced_test, y_train, y_test = train_test_split(X_ngram_reduced, y, test_size=0.2,
                                                                                random_state=42)
...
# Define the scores for the proposed features
proposed_scores = [accuracy_proposed, precision_proposed, recall_proposed, f1_score_proposed]

# Define the scores for the reduced N-gram features
ngram_scores = [accuracy_ngram_reduced, precision_ngram, recall_ngram, f1_score_ngram]
...
# Create the bar plot
plt.bar(r1, proposed_scores, color='b', width=bar_width, edgecolor='white', label='Proposed Features')
plt.bar(r2, ngram_scores, color='g', width=bar_width, edgecolor='white', label='Reduced N-gram Features')
    ...
    def __init__(self, sarcasm_detected):
        ...
        if sarcasm_detected:
            result_label.setText(
                '<html><body><span style="font-size:18pt; font-weight:bold; color:red;">This tweet is sarcastic</span></body></html>')
        else:
            result_label.setText(
                '<html><body><span style="font-size:18pt; font-weight:bold; color:green;">This tweet is not sarcastic</span></body></html>')
        movie.start()
